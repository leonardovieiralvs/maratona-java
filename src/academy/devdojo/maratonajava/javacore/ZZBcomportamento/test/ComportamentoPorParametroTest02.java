...
public class ComportamentoPorParametroTest02 {
    private static List<Car> cars = List.of(new Car("Yellow", 1999),
            new Car("Blue", 2010),
            new Car("Black", 2000));
...
//        List<Car> filterCarColor = filter(cars, zaza -> zaza.getColor().equals("Black"));
...
//        System.out.println(filterCarYear);
    ...
    private static List<Car> filter(List<Car> cars, CarPredicate carPredicate) {
        return cars.stream().filter(carPredicate::test).collect(Collectors.toList());
